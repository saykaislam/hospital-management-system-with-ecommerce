...
//        $orders=Order::where('user_id',Auth::id())->latest('created_at')->get();
    ...
    public function orderSubmit(Request $request){
        ...
        foreach ($orderProducts as $content) {
            ...
            $orderDetails->product_name = $content["product_name"];
    ...
    public function reviewStore(Request $request)
    {
        ...
        if ($order->delivery_status == 'Completed') {
            $check = ProductReview::where('user_id',Auth::id())->where('order_id',$request->order_id)->where('shop_id',$shop->id)->where('product_id',$request->product_id)->first();
            if (empty($check)){
    ...
    public function getReview(){
        $reviews = DB::table('product_reviews')
            ...
            ->join('shops','product_reviews.shop_id','=','shops.id')
            ->where('product_reviews.user_id','=',Auth::id())
            ->select('shops.name as shop_name','products.name as product_name','product_reviews.*')
            ->first();
        if (!empty($reviews))
        {
