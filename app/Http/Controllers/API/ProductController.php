...
class ProductController extends Controller
{
    ...
    public function getFlashDeals($id) {
        $shop = Shop::where('id',$id)->first();
        $flashDeal = FlashDeal::where('status',1)->where('user_id',$shop->user_id)->where('featured',1)->first();
        if (!empty($flashDeal))
        {
            return response()->json(['success'=>true,'response'=> $flashDeal], 200);
        }
        else{
            return response()->json(['success'=>false,'response'=> 'No Data Found!'], 404);
        }
    }
...
//    }
    ...
    public function variantPrice(Request $request)
    {
        ...
        if($str != null && $product->variant_product){
            return $product_stock = $product->stocks->where('variant1', $str)->first();
            if($product_stock){
