                ...
                <div class="row">
                    <div class="col-md-6" style="float: left">
                        <div class="biller-info">
                            <h4 class="d-block">Patient Name: {{ \App\User::where('id',$infos->user_id)->pluck('name')->first() }}</h4>
                            <span class="d-block text-muted">Age: ---</span> <br>
                            <span class="d-block text-muted">Phone: {{ \App\User::where('id',$infos->user_id)->pluck('phone')->first() }}</span>
...
{{--                            <span class="d-block text-muted">#INV-{{$infos->invoice}}</span>--}}
                            <span class="d-block text-muted">Phone: {{ Auth::User()->phone}}</span> <br>
                            <span class="d-block text-muted">Email: {{ Auth::User()->email}}</span>
                        </div>
                    </div>
                </div>
                                ...
                                @if(!empty($prescription_infos))
                                    @foreach($prescription_infos as $prescription_info)
                                        <tr>
                                            ...
                                            <td>
                                                {{ $prescription_info->morning ? 'morning: yes' : '' }}
                                                {{ $prescription_info->afternoon ? 'afternoon: yes' : '' }}
                                                {{ $prescription_info->evening ? 'evening: yes' : '' }}
                                                {{ $prescription_info->night ? 'night: yes' : '' }}
                                            </td>
                                        </tr>
                                    @endforeach
                                @endif
