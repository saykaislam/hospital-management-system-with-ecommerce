                        ...
                        <table id="order" class="table  " style="width:100%;overflow-x:auto;">
                            ...
                            @foreach($serviceOrders as $key => $order)
                                ...
                                <tr>
                                    ...
                                    {{--                                    <td class="text-center text-bold">{{$order->vendor->shop_name}}--}}
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if($order->delivery_status=="Completed")
{{--                                            <a class="edit-link" data-toggle="modal" href="#service_provider_review_{{$order->id}}"><i class="fa fa-edit mr-1"></i>Review</a>--}}
                                            <button title="Rate This Vendor" type = "button" class="btn btn-sm btn-info text-dark" data-toggle="modal" data-target="#service_provider_review_{{$order->id}}"><i class="fa fa-star text-light" aria-hidden="true"></i></button>
                                        @else
                                        @endif
                                    </td>
                                                        ...
                                                        <div class="dc-registerformgroup">
                                                            <div class="form-group ">
                                                                <label for="" class="text-dark" style="font-size: 17px">
                                                                    How would you rate
                                                                    @php
                                                                        $service_provider_name = \Illuminate\Support\Facades\DB::table('users')
                                                                            ->join('service_providers','users.id','service_providers.user_id')
                                                                            ->where('service_providers.id',$order->service_provider_id)
                                                                            ->pluck('users.name')
                                                                            ->first();
                                                                    @endphp
                                                                    <u>{{$service_provider_name ? $service_provider_name : ''}}</u>?
                                                                </label>
                                                                <div class="rate mt-2">
